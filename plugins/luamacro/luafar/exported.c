				...
				if (lua_type(L,-1)==LUA_TSTRING && lua_objlen(L,-1)==5 && !strcmp("panel",lua_tostring(L,-1)))
				{
					...
					if (lua_toboolean(L, -1))
					{
						struct FarMacroCall* fmc =
							(struct FarMacroCall*) malloc(sizeof(struct FarMacroCall) + sizeof(struct FarMacroValue));
						fmc->StructSize = sizeof(*fmc);
						fmc->Count = 1;
						fmc->Values = (struct FarMacroValue*)(fmc+1);
						fmc->Callback = FillFarMacroCall_Callback;
...
void getPluginMenuItems(lua_State* L, struct PluginMenuItem *pmi,
		const char* namestrings, const char* nameguids, int cpos)
{
	size_t count=0;
...
intptr_t LF_ProcessDialogEvent(lua_State* L, const struct ProcessDialogEventInfo *Info)
{
	...
	if (!(*Flags & PDF_DIALOGEVENTDRAWENABLE) && (
	...
	if (PushDMParams(L, fde->Msg, fde->Param1)) //+5
	{
		lua_setfield(L, -3, "Param1"); //+4
		lua_setfield(L, -2, "Msg");    //+3
	...
	if (pcall_msg(L, 2, 1) == 0)      //+1
	{
		if ((ret=lua_toboolean(L,-1)) != 0)
			fde->Result = PushDN
				? ProcessDNResult(L, fde->Msg, fde->Param2)
				: lua_tointeger(L,-1);
		lua_pop(L,1);
	}
	return ret;
...
int LF_GetGlobalInfo(lua_State* L, struct GlobalInfo *Info, const wchar_t *PluginDir)
{
	...
	if (lua_istable(L, -1))
		Info->MinFarVersion = MAKEFARVERSION(
			GetOptIntFromArray(L,1,0),
			GetOptIntFromArray(L,2,0),
		    GetOptIntFromArray(L,3,0),
			GetOptIntFromArray(L,4,0),
			GetOptIntFromArray(L,5,0));
	lua_pop(L, 1);
	lua_getfield(L, -1, "Version");
	if (lua_istable(L, -1))
		Info->Version = MAKEFARVERSION(
			GetOptIntFromArray(L,1,0),
			GetOptIntFromArray(L,2,0),
			GetOptIntFromArray(L,3,0),
			GetOptIntFromArray(L,4,0),
	...
	if (lua_type(L,-1) == LUA_TSTRING)
		Info->Guid = *(GUID*)lua_tostring(L,-1);
	lua_pop(L, 1);
	//--------------------------------------------------------------------------
