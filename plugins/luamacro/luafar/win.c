...
static int win_SetRegKey(lua_State *L)
{
	...
	else if(!strcmp("expandstring", DataType))
	{
		...
		if (hKey)
		{
			result = (ERROR_SUCCESS == RegSetValueExW(
				hKey, ValueName, 0, REG_EXPAND_SZ,
				(BYTE*)data, (DWORD)((1+len)*sizeof(wchar_t)))
					);
			RegCloseKey(hKey);
		}
	}
	else if(!strcmp("multistring", DataType))
	{
		...
		if (hKey)
		{
			result = (ERROR_SUCCESS == RegSetValueExW(hKey, ValueName, 0, REG_MULTI_SZ,
				(BYTE*)data, (DWORD)((1+len)*sizeof(wchar_t))));
			RegCloseKey(hKey);
		}
	}
