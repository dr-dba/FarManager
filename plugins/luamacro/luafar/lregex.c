...
int _regex_gmatch_closure(lua_State *L, int is_wide)
{
	...
	if ((pData->Position <= pData->Length) && RegExpControl(fr->hnd, RECTL_SEARCHEX, 0, pData))
	{
		...
		for (i = skip; i < pData->Count; i++)
		{
			...
			&& pData->Match[i].end >= pData->Match[i].start)
			{
				if (is_wide)
	...
	if (ftype == LUA_TSTRING)
	{
		...
		for (p = f; *p; p++)
		{
			if (*p == L'%')
			{
				...
				if (max_rep_capture < n)
					max_rep_capture = n;
			}
		}
	} else
	if (ftype != LUA_TTABLE && ftype != LUA_TFUNCTION)
	{
					...
					if (++i < flen)
					{
						...
						if (n >= 0)
						{
							...
							if (data.Match[n].start >= 0)
								luaL_addlstring(&out,
									(const char*)(s + data.Match[n].start),
